...
const StockTable = () => {
  ...
  const renderArrow = (current, previous) => {
        ...
        <Card className="mb-8">
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <motion.div whileHover={{ scale: 1.05 }} className="bg-green-100 p-4 rounded-lg flex items-center">
              ...
              <motion.div whileHover={{ scale: 1.05 }} className="bg-blue-100 p-4 rounded-lg flex items-center">
              ...
              <motion.div whileHover={{ scale: 1.05 }} className="bg-purple-100 p-4 rounded-lg flex items-center">
        ...
        {stockPredictions && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 bg-white p-6 rounded-xl shadow-lg">
            ...
            <ul className="grid grid-cols-2 gap-4">
              {stockPredictions.predictions.slice(0, 5).map((pred, i) => (
                <li key={i} className="p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-500">{pred.date}</p>
                  <p className="text-lg font-bold text-gray-800">${pred.price.toFixed(2)}</p>
                </li>
              ))}
            </ul>
            <h3 className="text-xl font-bold text-gray-800 mt-6">AI Insights</h3>
            {stockPredictions.trends.length > 0 ? (
              stockPredictions.trends.map((trend, idx) => (
                <p key={idx} className={`text-lg mt-2 ${trend.type === 'RISE' ? 'text-green-600' : 'text-red-600'}`}>  
